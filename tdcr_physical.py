...
Control interface:
  Command file:  tdcr_physical_cmd.json
    - theta: motor spool angles in radians (length = NUM_CABLES)
...
def _stl_height_mm(path: str) -> Optional[float]:
    """Read binary STL height extent in mm along Y."""
    try:
        ...
        with open(path, "rb") as f:
            ...
            for _ in range(n):
                f.read(12)  # normal
                for __ in range(3):
                    _, y, _z = struct.unpack("<3f", f.read(12))
                    ys.append(float(y))
                f.read(2)  # attr
        if not ys:
            return None
...
def createScene(root):
    # Keep all geometry behavior driven from sofa_tdcr_physical folder.
    # User request: use 4_sec_Backbone_3T model as one section and stack 4 copies.
    stl_candidates = [
        os.path.join(HERE, "4_sec_Backbone_3T.STL"),
        os.path.join(HERE, "4_sec_Backbone_3t.STL"),
        os.path.join(HERE, "4_sec_Backbone_3T.stl"),
        os.path.join(HERE, "4_sec_Backbone_3t.stl"),
    ]
    src_stl = next((p for p in stl_candidates if os.path.exists(p)), None)
    if src_stl is not None:
        # Use a neutral filename so tdcr.py takes the "replicate section mesh" path.
        section_stl = os.path.join(HERE, "tdcr_surface.stl")
        try:
            if (
                (not os.path.exists(section_stl))
                or os.path.getsize(section_stl) != os.path.getsize(src_stl)
                or os.path.getmtime(section_stl) < os.path.getmtime(src_stl)
            ):
                with open(src_stl, "rb") as sf, open(section_stl, "wb") as df:
                    df.write(sf.read())
        except Exception:
            section_stl = src_stl

        # Configure reference scene globals (without editing sofa_tdcr/tdcr.py).
        tdcr_ref.STL_FILE = section_stl
        h = _stl_height_mm(section_stl)
        if h is not None and h > 5.0:
            tdcr_ref.LENGTH = float(h)
        tdcr_ref.NUM_SECTIONS = int(NUM_SECTIONS)
        tdcr_ref.TENDONS_PER_SECTION = 3
        tdcr_ref.NUM_CABLES = int(NUM_CABLES)
        if hasattr(tdcr_ref, "CABLE_COLORS") and len(tdcr_ref.CABLE_COLORS) < tdcr_ref.NUM_CABLES:
            tdcr_ref.CABLE_COLORS = (tdcr_ref.CABLE_COLORS * ((tdcr_ref.NUM_CABLES // len(tdcr_ref.CABLE_COLORS)) + 1))[: tdcr_ref.NUM_CABLES]

        print(f"[PHYS] geometry source={src_stl}")
        print(f"[PHYS] section mesh={section_stl}")
        print(f"[PHYS] configured sections={tdcr_ref.NUM_SECTIONS}, section_height={tdcr_ref.LENGTH:.3f}mm")
    else:
        print("[PHYS] 4_sec_Backbone_3T STL not found in sofa_tdcr_physical; using reference defaults.")
